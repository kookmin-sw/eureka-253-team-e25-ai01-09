...
### 11주차 Team Mission
<details>
<summary> 대학원 선배 만나기 </summary>  

  
1. 학부와 대학원에서 배우는 내용의 차이는 무엇인가요?<br>
  - 고등학교 ~ 대학교에서는 검증되고 정제된 교과서같은 지식들을 배운다. 반면 대학원에서는 정제되지 않은 지식(지금 막 쏟아져 나오는 새로운 지식)을 배운다. 기존 논문의 허점을 찾아 반박하고 자신의 새로운 논문을 쓰는 과정이 핵심이다.
   
2. 인공지능학과는 대학원 진학이 필수인가요?<br>
  - AI분야는 현재 새로운 지식이 쏟아지고 있다. 학과에서는 60~70년대에 만들어진 자료구조, 알고리즘을 여전히 배우지만 LLM의 핵심 기술은 2017년에 등장했다. 이러한 점들 때문에 계속 새롭게 변화하는 인공지능에선 대학원이 필수라고 얘기가 나오는 것.<br>
  - AI 분야는 크게 ML리서치와 엔지니어로 나뉜다. 최근 테크캠프 등에서 AI 엔지니어를 많이 배출하지만, 이들 중 다수는 API를 활용하거나 프롬프트 엔지니어링을 하는 수준에 머무를 수 있다. 따라서 대학원에서 연구를 해본 경험은 실제 현업에서 모델의 근본적인 문제를 파악하고 아키텍처를 수정하는 등의 능력을 갖춘 차별점을 가진 엔지니어가 될 수 있다.
   
3. 인공지능 분야 이외에 타 전공과 융합하고 싶다고해도 대학원이 필수인가요?<br>
  - 대학원 진학이 필수는 아니다. 그러나 대학원에 가지 않는다면 그에 버금가는 스페셜리티를 가지고 있어야 한다. 따라서 부트캠프 출신과 차별화되고 높은 연봉에 걸맞는 역량을 갖추기 위해 대학원 경험이 가치가 있다.
   
4. AI대학원 대신 경영학 같은 특정 도메인을 정하고 AI를 접목하는 것은 어떤가요?<br>
  - 직업의 비가역성이라는 말이 있다. AI기술을 모르는 CEO는 개발자들이 거짓말을 하는지 간파하기 어렵다. 그러나 기술을 아는 CEO는 문제를 간파할 수 있다. 이처럼 남들이 어려워하는 것을 먼저 가지고 있으면 나중에 편하다. 따라서 지금 AI기술을 배워놓으면 미래에 스페셜리티를 가질 수 있다.
   
5. 대학원에 진학하기 전에 준비하면 좋은 것들은 무엇이 있나요?<br>
  - 학과공부 열심히 하기. 벼락치기는 남는게 없다. 배우는 내용을 정말로 이해하는게 중요함.<br> 
  - 검증된 논문을 읽어보면서, 모르는 부분을 채워가는 과정을 통해 공부해나가면 된다.<br>
  - 수학을 열심히 공부한다. 더 많은 도구를 가질 수 있다는 관점으로 공부하며, 파이토치 등을 이용하며 핵심연산이 어떻게 이루어지는지 알아본다.  

![Image](https://github.com/user-attachments/assets/d9ff8e0f-8cb8-498e-b0eb-5a39bfae6a28)

</details>

### 12주차 Team Mission
<details>
<summary> 교수님 만나기 </summary>  

  
1. 프론트엔드, 백엔드 등 세부 진로를 결정하는 기준은 무엇인가요?<br>
  - 무엇이 유망한지 타인에게 묻기보다는 직접 경험해봐야 알 수 있다. 수업 과제나 프로젝트를 수행하며 '내가 무엇을 더 공부하고 싶은지' 스스로 판단해야 한다.<br>
  - 취업 시 가장 중요한 것은 포트폴리오다. 단순히 학점이나 학교명을 넘어, 대학 생활 동안 어떤 프로젝트를 주도적으로 해왔는지를 보여주는 것이 핵심이다.<br>
  - 대학은 고등학교와 달리 정해진 목적이나 정답을 주지 않는다. 인생은 선택의 연속이므로, 스스로 결정하고 그에 따른 경험을 쌓는 연습을 지금부터 해야 한다.
   
2. AI가 코딩을 다 해주는 시대에 학부생이 갖춰야 할 역량은 무엇인가요?<br>
  - CS 관련 직군이라면 코딩 실력은 여전히 기본이자 필수다. AI 도구를 쓰더라도 그 결과물을 판단하고 수정하려면 전공 지식이 있어야 한다.<br>
  - 시니어 개발자는 AI를 써서 능률을 3배 이상 올리지만, 주니어는 배경지식이 없으면 AI가 짠 코드의 오류를 해결(디버깅)하지 못하고 적절한 질문(프롬프트)도 던지지 못해 능률이 오르지 않는다.<br>
  - AI라는 쉬운 도구에만 매몰되면, 복잡한 문제를 해결하는 사고력을 잃게 된다. 논리적으로 생각을 정리하여 글을 쓰고, 코드를 구조화하는 능력이 더욱 중요해졌다.
   
3. 효율적으로 코딩을 공부하는 방법은 무엇인가요?<br>
  - 책 한 권을 처음부터 끝까지 떼는 방식보다는, 내가 필요한 서비스(예: 알람 앱)를 직접 만들어보는 '프로젝트 중심'의 경험이 중요하다.<br>
  - AI를 단순한 '해결사'가 아닌 '개인 튜터'로 활용해야 한다. 막히는 부분을 질문하거나, 내 코드의 개선점을 물어보며 토론하듯 학습하는 것이 좋다.
   
4. 복수전공이나 타 전공 융합은 어떻게 선택해야 하나요?<br>
  - 현재 AI가 적용되지 않는 분야는 없다. 따라서 '무엇이 유리한가'보다 '내가 배운 CS 지식을 어느 도메인(디자인, 예술 등)에 적용하고 싶은가'라는 본인의 관심사가 더 중요하다.<br>
  - 학부 과정이나 융합 전공만으로 모든 지식을 완벽히 갖출 수는 없다. 공부에는 끝이 없으며, 부족한 부분은 대학원 진학이나 실무 경험을 통해 평생 채워나가야 한다.
   
5. 교수님이 대학원생을 선발할 때 중요하게 보는 기준은 무엇인가요?<br>
  - 성실함을 본다. 학점이 아주 높을 필요는 없지만, 학점은 학생이 수업을 얼마나 성실하게 듣고 참여했는지를 보여주는 지표가 된다.<br> 
  - 끈기와 열정(Motivation)이 중요하다. 연구는 생각보다 어렵고 힘든 과정이다. 어려운 문제에 부딪혔을 때 쉽게 포기하지 않고 끝까지 파고드는 집요함이 있는지를 가장 중요하게 평가한다.  

![Image](https://github.com/user-attachments/assets/7d9870f7-ea0b-4fd1-8a2e-185a967f294f)
